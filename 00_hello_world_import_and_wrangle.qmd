...
Scope note: Document very basic data wrangling code for observable js {ojs}. The assumption is that I know {tidyverse} code pretty well. Now I want to extend that knowledge so that i can, eventually, write reactive code and generate interactive observable documents.
...
3.  `aq.from()` is an [Arquero](https://uwdata.github.io/arquero/) function performed in an {ojs} code block
...
## load observable functions from d3 and aqquero
...
Plot.plot({
  marks: [
    Plot.dot(sw_no_jabba_aq, {x: "height", y: "mass", strokeOpacity: 0.5, r: 2}),
    Plot.linearRegressionY(sw_no_jabba_aq, {x: "height", y: "mass", stroke: "steelblue", ci: 0.95})
  ]
})
```
